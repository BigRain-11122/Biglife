#!/usr/bin/env python3
"""citizen_qa.py - whitelisted citizen Q&A harness (four-faces dialogue line, v0).

Extends cognition layer 3 (spotlight single-line) into a small multi-turn window:
whitelist = anchor citizens C-00010~C-00029; honored seats are persona-reserved
and rejected. Local Ollama only (local-compute law). Machine gate v0 subset:
G1 ascii/length, G2 placeholder, G3 celestial, G4 weather-consistency,
G5 time-of-day, G6 group-authority. Two regen attempts, then a deterministic
polite deflection. Contract: cognition/QA-WHITELIST.md v1.0 (CODEX 12 v3.4).

Usage:
  python -X utf8 citizen_qa.py --id C-00010 --ask "question" [--ask2 "..."]
  python -X utf8 citizen_qa.py --id C-00010 --ask "..." --json
...
def recent_memories(cid, district):
    ...
    if not mt:
        return []
    return re.findall(r"- (\d{4}-\d{2}-\d{2}) \u300c(.*?)\u300d", mt.group(1))[-3:]

def gate(text, sig, h):
    """Machine gate v0 subset. Returns violation key or None."""
...
def answer(row, sig, history, question):
    mem = recent_memories(row["id"], row.get("district"))
    h = datetime.datetime.now().hour
    out, gate_hits = "", []
    for attempt in range(3):
        raw = llm(build_prompt(row, sig, mem, history, question), 0.8 if attempt == 0 else 0.5)
...
def qc():
    ...
    p1 = build_prompt(row, sig, [], [], "q")
    p2 = build_prompt(row, sig, [], [], "q")
    if p1 != p2:
    ...
    if fails:
        ...
        return 1
    print("QC PASS (%d gate cases + prompt determinism + whitelist scope)" % len(cases))
    return 0
...
def main():
    ...
    if a.qc:
        sys.exit(qc())
    if not a.id or not a.ask:
        print("need --id and --ask"); sys.exit(2)
    if a.id in HONORED:
        print("rejected: honored seat (CEO persona-reserved)"); sys.exit(3)
    if a.id not in WHITELIST:
        print("rejected: citizen not in QA whitelist (see cognition/QA-WHITELIST.md)"); sys.exit(2)
    row = get_row(a.id)
    if not row:
        print("no citizen %s" % a.id); sys.exit(2)
    sig = real_signals()
    history, turns = [], []
    for q in (a.ask, a.ask2, a.ask3):
        if not q:
            continue
        ans, hits = answer(row, sig, history, q)
        history.append(("\u95ee", q))
        history.append(("\u7b54", ans))
        turns.append({"q": q, "a": ans, "gates": hits})
    out = {"id": row["id"], "name": row["name"], "species": row.get("species"),
           "district": row.get("district"), "weather": sig["weather"],
           "now": sig["now"], "turns": turns}
    if a.json:
