...
def ring_violations(line, sig):
    ...
    if m:
        ...
        if wind > 3 and re.search(r"风[^大]?不大|风[^大]?轻轻的", line):
            v.append("wind-under")
        if wind <= 3 and re.search(r"风[^小]?不小|风挺大|风好大", line):
            v.append("wind-over")
    elif re.search(r"风[^大]?不大|风[^大]?轻轻的|风[^小]?不小|风挺大|风好大", line):
