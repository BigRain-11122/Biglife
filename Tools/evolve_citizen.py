...
def build_prompt(cid, text, sig):
    ...
    return (f"你是超体宇宙城（一座赛博像素数字城市）的叙事市民「{cid}」。"
            ...
            f"硬约束（锚定律从严）：年轮中提及的具体事必须逐字来自上面的事件清单——只可截取清单原文短语，不得改写事实，不得添加清单之外的任何具体事（时间/人名/事件名）；泛泛的日常动作（开档、收摊、出摊）不算具体事。\n"
...
def main():
    ...
    if args.meet:
        ...
        prompt = (f"你是叙事编剧。城市真实事件：\n{ev}\n"
                  ...
                  f"硬约束（锚定律从严）：台词中提及的具体事必须逐字来自事件清单原文短语，不得添加清单外的具体事实。台词中禁止出现「居民甲」「居民乙」字样，直接以台词本身呈现。\n"
