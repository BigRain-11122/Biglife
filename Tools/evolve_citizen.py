...
v0.31 gap #40 (2026-09-26 R329): bare 太阳 escaped the cloud-side sky-mismatch
token list (R299 C-01507 「今早太阳刚刚好」 first strike; R328 C-01738
「太阳都偏西了」 recurrence -> gate close per the first-strike protocol).
R329 census: registry 太阳 hits = strike family + idiom face gene (比太阳
起得早/比太阳还早, 晨操领队 - C-00012 anchor et al.) + cover-read (云层
遮着太阳, C-01641 R314 判例) + 晒太阳 face rule-quotes (C-01492/C-01457
red line, v0.30(b) shape) + fed-event substrings + clear-fed (check off) +
historical rings (C-00331/C-00532/C-00540/C-00624/C-01457/C-01469/C-01665
not retro-repaired, #41 precedent). Narrow assertion: bare 太阳 against a
non-clear feed, minus four exemptions (cover-read / idiom / verbatim card
FACE fragment >=5 chars spanning 太阳 with rings stripped / verbatim
fed-event fragment). Batch prompt 晴系 token list gains 太阳 (generic list
only, no literal example - T-19 law).
v0.32 晴系 merge close (2026-09-26 R336): #41 insertion variants (天气清
C-01534 R302 first strike, 天头…清 C-01760 R330 dialect variant), #45
sun-intensity noun forms (晒得挺大 C-01748 R329 / 晒得挺厉害 C-01801 R335
recurrence) and #47 晴天 (C-01788 R333 first strike) all escaped the flat
cloud-side list -> ONE merged exemption-aware check (#40 shape, R335 verdict:
gate 编码整轮预算). R336 census: #46-widened window 1 hit = the violation
itself; 晴天 1 = clear-window historical (not retro, #41 precedent); 天头…清
0; 晒得 0 - 零误伤. 天气清(?![清爽]) keeps the 清爽/清清爽爽 body-sense
families legal (R334 five-family exemption census). Exemptions: verbatim
card FACE fragment (>=4 chars spanning the token, rings stripped - 晴天
卖伞骨 rule-quote gene family) + verbatim fed-event fragment + clear-fed
(check off). #46 (R332 C-01778 first strike, R336 C-01813 「天气正好cloud」
recurrence -> close per protocol): the #30 leak pattern widens to a
<=4-char insertion window before the raw token (R333 pre-registration).
Batch prompt 晴系 token list gains the generic tokens (no literal example).
v0.33 gap #48 清朗 close (2026-09-26 R338): 清朗 = 晴朗 with 晴 swapped for
清, a #34 sibling form escaping the flat cloud-side list. C-01562 R306 first
strike was card-fix only; C-01830 R338 recurrence -> protocol gate close.
Census scan: 10 hits = the violation itself + 1 clear-window legal (C-01672
...
(no token list face).
v0.35 gap #50 trigger-domain close (2026-09-26 R386): the flat 晴系 list only
fired on cloud|阴 feeds, so rain/thunder/mist windows carried ZERO check -
C-01869 R343 first strike (清气朗 under rain), R385 rain-window 窝案
C-02153/C-02154/C-02167/C-02170 (天清气朗 x4) -> recurrence protocol gate
close. Trigger aligned with the #40/v0.30/v0.32 family condition (non-clear
feed), which closes the whole non-clear family in one semantics, not just
rain. Census FP scan (R287 范式): face-section hits (rings stripped) = 0 -
zero face-catchphrase families among the flat tokens, no new exemption
needed; all registry hits are historical rings (clear-fed legal at write
time + already card-fixed 窝案 lines), not retro (#41 precedent); the
「天气正好/天气挺好」 subjective-reading family is NOT in the token list
(red line held). Batch + meet prompt 晴系/晒 clauses' trigger wording widened
cloud/阴 -> 非晴天气 (generic list unchanged, no literal negative example -
T-19 law).
...
def ring_violations(line, sig):
    ...
    # gap #48 (2026-09-26): 清朗 - 晴朗 with 晴 swapped for 清 (#34 sibling form,
    # flat-list escape); C-01562 R306 first strike (card-fix only), C-01830
    # R338 recurrence -> gate close per protocol. Census scan: 10 hits = the
    # violation itself + 1 clear-window legal (C-01672 R318) + 8 historical
    # 前朝 rings (not retro, #41 precedent) - zero face catchphrases 零误伤.
    # v0.35 gap #50 (2026-09-26 R386): trigger domain was cloud|阴 only, so
    # rain/thunder/mist feeds reached this check NOT - C-01869 R343 first
    # strike + R385 rain-window 窝案 (C-02153/02154/02167/02170 天清气朗)
    # -> trigger aligned with the #40/v0.30/v0.32 non-clear family condition.
    # Census FP scan: face-section hits = 0 (no new exemption needed);
    # historical rings not retro-repaired (#41 precedent).
    if wx and not re.search(r"clear|晴", wx, re.I) and re.search(r"天晴|晴着|今天清[着气得]|(?<!今)天清|晴空|晴朗|阳光|晴得|清气朗|清朗", line):
        v.append("sky-mismatch")
    ...
    # v0.21 gap #30 + v0.32 gap #46 (2026-09-26 R336): raw weather-kind token
    # leak - the fed weather string is "cloud 30.5°C wind 9.9m/s" and the LLM
    # sometimes copies the kind verbatim into the ring (「天气cloud」 C-00635
    # first strike R183; C-00659 + C-00711 same-batch strikes R194 = gate close;
    # #46 insertion variant 「天气正好cloud」 C-01778 R332 first strike,
    # C-01813 R336 recurrence -> window widened to <=4 chars between 天气 and
    # the token, R333 pre-registration). Census R336: the widened window hits
    # only the violation itself (零误伤); Chinese body-sense words stay
    # untouched; the ASCII token list is closed so any hit is a leak by
    # definition (拟真防火墙).
    if re.search(r"天气[^，。！？；;\n]{0,4}(?:cloud|clear|rain|snow|overcast|mist|fog|drizzle|showers?)", line, re.I):
    ...
    if ct is not None and wx and not re.search(r"clear|晴", wx, re.I):
        ...
        for m in re.finditer(r"正好[^，。！？；;\n]{0,12}晒|晒[^，。！？；;\n]{0,8}正好|适合[^，。！？；;\n]{0,12}晒|晒[^，。！？；;\n]{0,8}适合|合适[^，。！？；;\n]{0,12}晒|晒[^，。！？；;\n]{0,8}合适|该[^，。！？；;\n]{0,10}晒[^，。！？；;\n]{0,4}了", line):
            ...
            v.append("sun-fit")
            break
    # v0.31 gap #40 (2026-09-26 R329): bare 太阳 under a non-clear feed is a
    # sun-state assertion the feed never anchors (R299 C-01507 first strike,
    # R328 C-01738 recurrence -> gate close). Four exemptions (R329 census):
    #   (a) cover-read - 遮/挡/蔽+太阳 in either order, negation-guarded so
    #       遮不住太阳 (negated cover = sun claim) stays flagged;
    #   (b) idiom face gene - 比…太阳…早/起 (比太阳起得早, 晨操领队);
    #   (c) verbatim card FACE fragment (>=4 chars spanning the 太阳, rings
    #       stripped) - 晒太阳 face families (C-01492/C-01457 red line);
    #   (d) verbatim fed-event fragment (same shape, forward window covers
    #       太阳能-type tokens).
    # Clear-fed lines never reach the check; historical rings NOT
    # retro-repaired (原文保全, #41 precedent). ct absent -> whole check off
    # (exemption (c) needs the card face; #18/#24/#25 + v0.30 sun-fit design).
    if ct is not None and wx and not re.search(r"clear|晴", wx, re.I) and "太阳" in line:
        if not (re.search(r"(?<![不没无])[遮挡蔽][住着了]{0,2}太阳|把太阳[遮挡]|太阳[被把][遮挡]", line)
                or re.search(r"比[^，。！？；;\n]{0,6}太阳[^，。！？；;\n]{0,8}(早|起)", line)):
            face40 = re.sub(r"\*\*年轮\*\*\n(?:- .*\n?)+", "", ct) if ct is not None else ""
            ev40 = "\n".join(sig.get("events") or [])
            hit40 = False
            for m in re.finditer("太阳", line):
                i = m.start()
                if any(line[s:e] in src for src in (face40, ev40)
                       for s in range(max(0, i - 11), i + 1)
                       for e in range(i + 2, min(len(line), i + 8) + 1)
                       if e - s >= 4):
                    hit40 = True
                    break
            if not hit40:
                v.append("sky-mismatch")
    # v0.32 晴系 merge close (2026-09-26 R336): #41 insertion variants
    # (天气清 / 天头…清), #45 sun-intensity noun forms (晒得…) and #47 晴天
    # escaped the flat cloud-side list (#19/#23/#26/#34 family). ONE merged
    # exemption-aware check (#40 shape). 天气清(?![清爽]) keeps the 清爽/
    # 清清爽爽 body-sense families legal (R334 five-family census);
    # R336 census: 晴天 1 = clear-window historical (not retro), 天头…清 0,
    # 晒得 0 - 零误伤. Exemptions:
    ...
    if ct is not None and wx and not re.search(r"clear|晴", wx, re.I):
        ...
        for m in re.finditer(r"晴天|天气[^，。！？；;\n]{0,4}清(?![爽凉清早晨])|天头[^，。！？；;\n]{0,2}清|晒得", line):
            ...
            if any(line[s:e] in src for src in (face32, ev32)
                   ...
                   for e in range(i + 2, min(len(line), i + 8) + 1)
                   if e - s >= 4):
                continue  # (c)/(d) verbatim card-face / fed-event fragment
            hit32 = True
            break
        if hit32:
            v.append("sky-mismatch")
...
def build_prompt(cid, text, sig):
    ...
    city_blk = (cd + "\n") if cd else ""
    # T-20260926-12 (L4 目标系统): optional 本月目标达成 block. Empty string ->
    # byte-identical to the pre-wiring build (zero-drift law, same as rumor/city).
    gl = (sig.get("goal") or "").strip()
    goal_blk = (gl + "\n") if gl else ""
...
def main():
    ...
    for cid in due:
        ...
        try:
            ...
            # T-20260926-12 (L4 目标系统·D-07 优先级序4): done 态达成候选块.
            # 试点门 md5(cid)%200 (~50 席) 外恒 "" -> build_prompt 零漂移.
            try:
                import goals as _goals
                sig["goal"] = _goals.goal_line_for(cid, sig)
            except Exception:
                sig["goal"] = ""
