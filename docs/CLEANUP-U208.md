...
## 分步②续（R237·2026-09-25 交付·候选 #2/#5 定谳=7/7 全毕）

> 本节按原文保全铁律=追加不改写已落段落；证据=当轮机械扫描实读（临时件跑后即删·py_compile 过）。

### 候选 #2：CODEX §十二 T2 版本行叠加——定谳：**非冲突·行序=登记时序非版本序**
- 条目号唯一性全过：v1.1~v3.17 全量登记号零重复（「；vN.M 标题」锚提取 33 条目逐条在册；撞号双让路后 v3.3=开场句软引导/v3.9=声音面/v3.10=问答白名单/v3.11=关系面契约位/v3.12=画像面/v3.13=记忆检索各归其主·v3.4/v3.5=让路弃用号仅存于实录行=正确不留尸体条目）。
- 行序非单调（文档序=…v3.3→v3.9→v3.10→v3.11→v3.6→v3.7→v3.8→v3.12…）=撞号双让路时序产物（我方原拟 v3.3/3.4/3.5→顺移 v3.6~3.8 被循环续占→终号 v3.9~3.11），非规则冲突。
- 时效态：每条自带否决窗日期，零超期未标。
- 分步③/④ 动作：CODEX 原文零删改；分步④ 索引面给正序版本索引（v1.1→v3.17）+一行「§十二行序=登记时序非版本序」注记。

### 候选 #5：Tools 21 py 头注版本号 ↔ §十二 对照——定谳：**3 处失效指针（撞号窗口产物·当轮已修）+2 类常态非缺口**
- **失效指针 3 处（当轮修复=注释行一行改·py_compile 过·全工具 T2 指针终扫 4/4 一致）**：
  1. `voice_manifest.py`「T2 v3.3」→**v3.9**（声音面·VOICE-POOL.md 已正确标 v3.9 在案）；
  2. `persona_enrich.py`「T2 v3.9」→**v3.12**（画像面·PERSONA-v2.md 已正确标 v3.12 在案）；
  3. `memory_index.py`「T2 v3.10」→**v3.13**（记忆检索与问答服务面·§十二 v3.13 条目点名本件）。
  根因=撞号终裁时「引用面全同步改毕（VOICE-POOL/QA-WHITELIST/R-件/HQ 行/本板行）」漏了 py 头注释面；修复后全工具 T2 指针与 §十二归属一致（citizen_qa.py v3.10=问答白名单面·其对口契约 QA-WHITELIST.md 亦标 v3.10 在案）。
- 常态非缺口 1：**自引可缺省**——8 py（spotlight/draw/pool_gen/qc_census/behavior/atlas_manifest/city_broadcast/add_honored_seats）头注无版本自引；登记面唯一=CODEX §十二（如 behavior.py=v2.2 条目点名），缺省非冲突。
- 常态非缺口 2：**机审门 v0.x 系=工具内版本史非 T2 面**——evolve_citizen.py 头注 v0.2~v0.22 为机审门演进史（判例承载=本板台账），与 §十二 T2 登记轴不同物·零冲突。

### 分步②收口计数（R236 节+本节合计）
冲突定谳：真实冲突 2（#1 时效态三层·#6 同域双载）+失效指针修复 3（#5 内·当轮毕）+已自带解决 3（#3/#4/#7）+非冲突 1（#2）+常态非缺口 2 类（#5 余面）→ **7/7 候选全定谳**。全域同义词余扫（SILICON-LIFE ↔ CODEX 条款重复度/cognition 契约间重叠）→ 随分步③ 终裁窗（09-27~28）一并扫。

## 候选补充登记（R245·2026-09-25）

> 原文保全铁律=追加不改写已落段落。来源=R243 台账记档行（年轮生成面观察→候选登记·非文档冲突件）。

- **候选 #8：城区名口语缩写泛化**——年轮面 C-01147 R243「G区」=GAME 城口语缩写首例（判例族=C-01054 R218「北区」泛化合法·非编造地名·正典城区名=CODEX §城区表）。**定谳：非文档冲突**（口语缩写=生成面风格现象非规则文档面）；动作=归入全域同义词余扫词汇观察项（09-27~28 窗随扫·缩写族如后续扩散再议词域正典化）。
