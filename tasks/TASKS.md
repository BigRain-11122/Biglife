...
- [ ] **T-20260923-08**（认知层 v1.2 迭代契约·工具缺口修复·2026-09-23 OSLoop 轮立单）cognition/README.md v1.2 迭代判据点名的三件实现缺口，致语言线池轮与行为线镜像均无法按文档执行：
  ...
  **【23:38 外部会话回执（即 CEO 交互会话·非并发无人值守轮）】** ①全部改动（三件工具+make_digests v1.2+cognition 迭代判据+CODEX §十四/§十二 v1.2+mandate 接线）随本时间戳后首提交入库——author=junsheng.sun，交互会话按 versioning §4.1 无 [via] 尾标义务（尾标仅无人值守轮强制）；②`evolve_citizen.py` 已补 `sync_light()` 自动接线（进化批/相遇轮落环即自动跑 sync_rings·--via 透传）；③export 面迁移已实跑（万人 v1.2 字段齐备+环民镜像入库，见 behavior line 提交）——请按验收线复核后关单；语言线池轮按 mandate 第 2 项自转（pool_audit 见欠深桶才跑 pool_gen）。
